...
required=(
  SUPABASE_PROJECT_REF SUPABASE_ACCESS_TOKEN SUPABASE_URL
...
SUPABASE_CLIENT_KEY="${SUPABASE_ANON_KEY:-${SUPABASE_PUBLISHABLE_KEY:-}}"
if [ -z "${SUPABASE_CLIENT_KEY}" ]; then
  echo "Missing from .env: SUPABASE_ANON_KEY or SUPABASE_PUBLISHABLE_KEY"
  exit 1
fi
...
supabase secrets set \
  ...
  ${NEWSAPI_KEY:+NEWSAPI_KEY="$NEWSAPI_KEY"} \
